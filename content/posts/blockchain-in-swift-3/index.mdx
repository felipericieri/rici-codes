...
This article is part of a serie of Building a Blockchain in Swift. You can find [here](/building-a-blockchain-in-swift-part1) the Part. 1 and [here](/building-a-blockchain-in-swift-part2) the Part. 2.
...
final class MiningService {
  ...
  /// 1. How many zeroes we'll need to regard so the hash can be accepted
  ...
  /// 2. Mines a the next block in the blockchain
  ...
  /// 5. Generate Hash for Block according to the difficulty of this blockchain
  func generateHash(for block: Block) -> String {
    print("🔑 Finding the hash for block at height \(block.index)...")
    var hash = block.key.toSHA1()
    
    // 6. Search for a hash prefix according to the Blockchain difficulty
...
3. We start we an instance of the next block, and then add the transactions to the block.
...
As you can see, the way we go to find another hash is to increment our `nonce` value. This is exactly the purpose we have it there. Is just a random number that helps us to find a hash that solves the blockchain's mathematical problem. We can't really change any other value in the block, it should stay as it is.

This is a very simple way to implement the Proof-of-Work protocol, but not so efficient. You see, you don't have any guarantee that the `nonce` you're using next is the right one, so there's no other way apart from trial-and-error to find this value. Feel free to explore other ways to implement yours! 
...
```
🔑 Finding the hash for block at height 0...
...
> My machine needed to run the function 218x to be able to find a hash for the genesis block! Picture how many time would be consumed to find a block with today's difficulty! 😬
...
```
⛏ Start mining block at height 1...
🔑 Finding the hash for block at height 1...
...
🙌 Hash found! - 00e97b38c191495227b6d3cb5fdaad175f947e21
```

> This time the function was ran 525 times! 😱
...
Woohoo! We've just written our first Blockchain in Swift, using Proof-of-Work! As I've mentioned, we aren't using the most effective algorithm to find the hash in the market, but that's a great start. That's also not the only protocol out there, most advanced blockchains uses protocols such as **Proof-of-Stake**, **Proof-of-Authority**, **Proof-of-Space**, and many others. 

Hope you had as much fun as I had writing this. Thanks for reading! If you have any suggestions or questions, please hit me in [twitter](https://twitter.com/ricicodes)!
