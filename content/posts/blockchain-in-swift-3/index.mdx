...
In our payload, we can't really change a thing. We should keep transactions integrity, it should be added to the chain as it is. The way we use to find another hash is to increment our `nonce` value. This is precisely the only purpose we have it there. It's just a random number that helps us to find a hash that solves the blockchain's mathematical problem.

In a live blockchain, this algorithm isn't part of the software. We are just putting it here for the experiment. This is the Node duty - and where the gold is -, so it uses it's own solution.
...
This is a very simple way to implement the Proof-of-Work protocol, but not so efficient. So feel free to explore other ways to implement yours! 
...
We've just written our first Blockchain in Swift using Proof-of-Work! As I've mentioned, we aren't using the most effective algorithm to find the hash in the market, but that's a great start. That isn't the only protocol out there, though. Most advanced blockchains use protocols such as **Proof-of-Stake**, **Proof-of-Authority**, **Proof-of-Space**, and many others.
