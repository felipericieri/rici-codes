...
Can you tell which one below is synchronous and which one is asynchronous?
...
The answer is - there's no way to know by this code.

Wait, wat?

An escaping closure does not mean a closure _will_ perform an asynchronous operation. An escaping closure just means that the closure will _escape_ its context, or simply it can not to be consumed within the block. But you still can have synchronous tasks running in it. It all depends of your implementation.
